import type { CoreEvent, SubscriptionId } from '#src/generated/protocol';
...
export class TimelineEventRouter<Owner> {
  ...
  route(event: TimelineEvent): Owner | null {
    ...
    if (this.pendingOwners.size === 0 || this.overflowed) return null;

    if (this.pendingEventCount >= MAX_PENDING_EVENTS) {
      this.pending.clear();
  ...
  claim(subscription: SubscriptionId, owner: Owner): TimelineEvent[] | 'overflow' | null {
