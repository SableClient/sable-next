...
impl SableCore {
    ...
    pub async fn submit_command(&self, command: String) -> Result<String, String> {
        ...
        match self.core.dispatch(command).await {
            Ok(response) => Ok(serde_json::to_string(&response).expect("CommandOk serializes")),
            Err(error) => Err(serde_json::to_string(&error).expect("CommandErr serializes")),
        }
    }
    ...
    pub async fn fetch_media(
        ...
        self.core
            ...
            .await
            .map_err(|error| serde_json::to_string(&error).expect("CommandErr serializes"))
    }
    ...
    pub fn subscribe_events(&mut self, on_event: Function) -> Result<(), JsValue> {
        ...
        wasm_bindgen_futures::spawn_local(async move {
            while let Some(event) = events.next().await {
                let json = serde_json::to_string(&event).expect("CoreEvent serializes");
                if on_event
...
mod tokio_stream_compat {
    ...
    impl UnboundedReceiverStream {
        pub fn new(receiver: UnboundedReceiver<CoreEvent>) -> Self {
            Self(receiver)
        }
    }
